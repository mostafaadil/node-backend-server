...
exports.selectStatesByFilter = async (req, res, next) => {
  var params = {
    limit: 10, page: 1, constrains: { name: "" },
    relations: [{ model: conn.cities }, { model: conn.unvarsecites },]
  }
  const result = await filter.filter("states", params)
  if (result) {
...
exports.search = async (req, res, next) => {
  ...
  await conn.states.findAll({
    limit: req.body.limit,
    offset: offset,
    include: ['cities', 'unvarsecites',],
    where: {
      [searchCol]: {
        [Op.like]: '%' + search + '%'
      }
...
exports.getStates = async (req, res, next) => {
  try {
    const result = await conn.states.findAll()
    if (result)
      res.status(200).json({ status: true, data: result })
    else {
      res.status(200).json({ status: false, message: `No data founded` })
...
exports.createStates = async (req, res, next) => {
  try {
    const result = await conn.states.create(req.body)
    if (result)
...
exports.paginate = async (req, res, next) => {
  try {
    ...
    var result = await conn.states.findAll({
      order: [["id", "DESC"]],
      include: [{ model: conn.countries, as: "country", attributes: ["name"] }],
      offset: offset,
      limit: req.body.limit,
      subQuery: false,
    })
    if (result) {
      var count = await conn.states.findAll();
...
//@route  GET
//@access Public
exports.getStatesById = async (req, res, next) => {
  try {
    const result = await conn.states.findOne({ where: { id: req.params.id } })
    if (result.length != 0)
      res.status(200).json({ status: true, data: result })
    else {
      res.status(200).json({ status: false, message: `No data founded` })
...
//@route  Put
//@access Public
exports.updateStates = async (req, res, next) => {
  try {
    const isUpdated = await conn.states.update(req.body, { where: { id: req.params.id } })
    if (isUpdated)
      res.status(200).json({ status: true, data: req.body })
    else {
...
exports.deleteStates = async (req, res, next) => {
  try {
    const isDeleted = await conn.states.destroy({ where: { id: req.params.id } })
    if (isDeleted)
