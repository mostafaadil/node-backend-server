...
exports.selectCountriesByFilter=async (req,res,next)=>{
  ...
  relations:[{model:conn.states},{model:conn.unvarsecites},{model:conn.users},]
  }
...
exports.search = async (req, res, next) => {
  ...
  await conn.countries.findAll({
    limit: req.body.limit,
    offset:offset,
    include:['states','unvarsecites','users',],
    where: {
      [searchCol]: {
        [Op.like]: '%' + search + '%'
      }
...
exports.paginate = async (req, res, next) => {
 try {
    ...
    var result = await conn.countries.findAll({
      order: [["id", "DESC"]],
      offset: offset,
      limit: req.body.limit,
      subQuery: false,
    ...
    if (result) {
      var count = await conn.countries.findAll();
...
exports.getCountriesById =async (req, res, next) => {
  try{
      const result=await conn.countries.findOne({where:{id:req.params.id}})
      if(result.length!=0)
         res.status(200).json({ status: true, data: result })
         else{
           res.status(200).json({ status: false, message: `No data founded` })
...
exports.updateCountries =async (req, res, next) => {
  try{
    const isUpdated=await conn.countries.update(req.body,{where:{id:req.params.id}})
    if(isUpdated)
      res.status(200).json({ status: true, data: req.body })
      else{
...
exports.deleteCountries = async (req, res, next) => {
   try{
    const isDeleted=await conn.countries.destroy({where:{id:req.params.id}})
    if(isDeleted)
