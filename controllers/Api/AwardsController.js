...
exports.selectAwardsByFilter = async (req, res, next) => {
  var params = {
    limit: 10, page: 1, constrains: { name: "" },
    relations: [{ model: conn.poster_awareds },]
  }
  const result = await filter.filter("awards", params)
  if (result) {
...
exports.search = async (req, res, next) => {
  ...
  await conn.awards.findAll({
    limit: req.body.limit,
    offset: offset,
    include: ['poster_awareds',],
    where: {
      [searchCol]: {
        [Op.like]: '%' + search + '%'
      }
...
exports.getAwards = async (req, res, next) => {
  try {
    const result = await conn.awards.findAll()
    if (result.length != 0)
      res.status(200).json({ status: true, data: result })
    else {
      res.status(200).json({ status: false, message: `No data founded` })
...
exports.createAwards = async (req, res, next) => {
  try {
    const result = await conn.awards.create(req.body)
    if (result)
...
exports.paginate = async (req, res, next) => {
  try {
    ...
    var result = await conn.awards.findAll({
      order: [["id", "DESC"]],
      offset: offset,
      limit: req.body.limit,
      subQuery: false,
    ...
    if (result) {
      var count = await conn.awards.findAll();
...
//@route  GET
//@access Public
exports.getAwardsById = async (req, res, next) => {
  try {
    const result = await conn.awards.findOne({ where: { id: req.params.id } })
    if (result.length != 0)
      res.status(200).json({ status: true, data: result })
    else {
      res.status(200).json({ status: false, message: `No data founded` })
...
//@route  Put
//@access Public
exports.updateAwards = async (req, res, next) => {
  try {
    const isUpdated = await conn.awards.update(req.body, { where: { id: req.params.id } })
    if (isUpdated)
      res.status(200).json({ status: true, data: req.body })
    else {
...
//@route  Delete
//@access Public
exports.deleteAwards = async (req, res, next) => {
  try {
    const isDeleted = await conn.awards.destroy({ where: { id: req.params.id } })
    if (isDeleted)
