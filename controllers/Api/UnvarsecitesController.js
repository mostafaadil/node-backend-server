...
exports.selectUnvarsecitesByFilter = async (req, res, next) => {
  var params = {
    limit: 10, page: 1, constrains: { name: "" },

  }
  const result = await filter.filter("unvarsecites", params)
  if (result) {
...
exports.search = async (req, res, next) => {
  ...
  await conn.unvarsecites.findAll({
    ...
    var count = conn.unvarsecites.findAll();
...
//@route  GET
//@access Public
exports.getUnvarsecites = async (req, res, next) => {
  try {
    const result = await conn.unvarsecites.findAll()
    if (result)
      res.status(200).json({ status: true, data: result })
    else {
      res.status(200).json({ status: false, message: `No data founded` })
...
exports.createUnvarsecites = async (req, res, next) => {
  try {
    const result = await conn.unvarsecites.create(req.body)
    if (result)
...
exports.paginate = async (req, res, next) => {
  try {
    ...
    var result = await conn.unvarsecites.findAll({
      order: [["id", "DESC"]],
      include: ["city","country","state","user"],
      offset: offset,
      limit: req.body.limit,
      subQuery: false,
    })
    ...
    if (result) {
      var count = await conn.unvarsecites.findAll();
...
//@route  GET
//@access Public
exports.getUnvarsecitesById = async (req, res, next) => {
  try {
    const result = await conn.unvarsecites.findOne({ where: { id: req.params.id } })
    if (result.length != 0)
      res.status(200).json({ status: true, data: result })
    else {
      res.status(200).json({ status: false, message: `No data founded` })
...
//@route  Put
//@access Public
exports.updateUnvarsecites = async (req, res, next) => {
  try {
    const isUpdated = await conn.unvarsecites.update(req.body, { where: { id: req.params.id } })
    if (isUpdated)
      res.status(200).json({ status: true, data: req.body })
    else {
...
//@route  Delete
//@access Public
exports.deleteUnvarsecites = async (req, res, next) => {
  try {
    const isDeleted = await conn.unvarsecites.destroy({ where: { id: req.params.id } })
    if (isDeleted)
